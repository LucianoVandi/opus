...
class PageController extends Controller
{
    ...
    public function show(Team $team, Space $space, Wiki $wiki, Page $page)
    {
        ...
        return view('page.index', compact('team', 'pageTags', 'page', 'wiki', 'space', 'isUserLikeWiki', 'isUserLikePage', 'isPageInReadList', 'attachments', 'attachable'));
    }
    ...
    public function generatePdf(Team $team, Space $space, Wiki $wiki, Page $page)
    {
        ...
        $dompdf->loadHtml($page->description);
        $dompdf->render();
        return response()->make($dompdf->output())
            ->header("Content-type","application/pdf")
            ->header("Content-disposition","attachment; filename=\"".$page->name.".pdf"."\"");
    }
