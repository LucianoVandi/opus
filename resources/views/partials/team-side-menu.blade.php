<div class="side-menu hidden-sm hidden-xs">
	<div class="side-menu-inner">
		<ul class="list-unstyled side-menu-top">
			<li class="nav-header">{{_i('Quick Links')}}</li>
			<li class="item {{ (Route::currentRouteName() == 'dashboard' ? 'active' : '') }}">
				<a href="{{ route('dashboard', [$team->slug]) }}">
					...
					<span class="item-name">{{_i('Activities')}}</span>
				</a>
			</li>
		...
		<div class="side-menu-wiki-list">
			<ul class="list-unstyled" id="categories-list">
				<li class="nav-header" style="position: relative; margin-bottom: 12px;">
					{{_i('Spaces')}} <span style="font-weight: 900; font-size: 15px;">-</span> {{ $spaces->count() }} 
					<a href="{{ route('spaces.create', [ $team->slug ]) }}" style="position: absolute; right: 10px; top: 5px; color: #9c9c9c;">
						<i class="fa fa-plus fa-fw" data-toggle="tooltip" data-placement="top" title="{{_i('Create space')}}"></i>
					</a>
				</li>
				@if($spaces->count() > 0)
					<li>
						<div class="form-group with-icon" style="margin-bottom: 5px;">
							<input type="text" class="form-control fuzzy-search" placeholder="{{_i('Filter')}}...">
		        ...
		        @else
		           	<li class="text-center text-muted">
		           		<h1 class="nothing-found side" style="margin-top: 20px; "><i class="fa fa-exclamation-triangle fa-fw icon"></i> {{_i('Nothing found')}}</h1>
		           	</li>
		        @endif
			</ul>
		</div>
