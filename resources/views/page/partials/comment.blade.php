...
<div class="wiki-comment-form">
    <form action="{{ route('pages.comments.store', [$team->slug, $space->slug, $wiki->slug, $page->slug]) }}" method="POST">    
        <div class="media">
            ...
            <div class="media-body">
                <div class="form-group {{ $errors->has('comment') ? 'has-error' : '' }}" style="margin-bottom: 13px;">
                    <textarea name="comment" class="form-control" id="comment-input-textarea" placeholder="{{_i('Write a comment')}}"></textarea>
        ...
        <input type="submit" class="btn btn-primary pull-right" value="{{_i('Submit')}}">
        <div class="clearfix"></div>
    </form>
</div>