            ...
            <div class="tab-content">
                <div class="s-groups-con">
                    <div class="s-group-header">
                        <div class="header">
                            <h2 style="margin-bottom: 35px;">{{_i('Edit Role')}}</h2>
                        </div>
                    </div>
                    <div class="">
                        <form action="{{ route('roles.update', [ $team->slug, $role->slug ]) }}" method="POST" role="form">
                            ...
                            <div class="form-group {{ $errors->has('role_name') ? 'has-error' : '' }} with-icon">
                                <label for="role-name" class="control-label">{{_i('Role Name')}}</label>
                            ...
                            <div class="form-group">
                                <label style="margin-top: 5px; margin-bottom: 0px;">{{_i('Permissions')}}</label>
                            ...
                            <ul class="list-unstyled list-inline pull-right">
                                <li>
                                    <a href="{{ route('roles.index', [$team->slug, ])  }}" class="btn btn-default pull-right">{{_i('Close')}}</a>
                                </li>
                                <li>
                                    <button type="submit" class="btn btn-success pull-right">{{_i('Update')}}</button>
                                </li>
                            </ul>
