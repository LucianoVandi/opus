...
@section('content')
	<div class="home-con">
		...
		<div class="home-page login-page">
			<div class="login-form-con" style="margin-bottom: 24px;">
                ...
                <form action="{{ route('team.postlogin') }}" method="POST" role="form">
		            ...
		            <div class="form-group {{ $errors->has('password') ? 'has-error' : '' }}">
		            	<a href="{{ route('password.request') }}" class="text-muted pull-right" style="font-size: 14px;">{{_i('Forgot your password?')}}</a>
			...
			<p class="text-center" style="font-size: 14px;"><span class="text-muted" style="margin-left: 15px;">{{_i('Don\'t have a team?')}}</span> <a href="{{ route('team.create') }}">{{_i('Create now')}}</a>.</p>
		</div>
	</div>
@endsection()