			...
			<div class="tab-content">
				<div class="team-info">
                    ...
                    <p class="text-muted action-info">
                        {{_i('Renaming team name will also change the team URL and will render all bookmarks to team pages invalid.')}}        
                    </p>
                    <form action="{{ route('teams.update', [ $team->slug ]) }}" method="POST" class="form-inline" role="form">
                        ...
                        <button type="submit" class="btn btn-success pull-right">{{_i('Save')}}</button>
                ...
                <div class="delete-team">
                    <h2>{{_i('Delete Team')}}</h2>
                    <p class="text-muted action-info">
                        {{_i('Deleting this team will permanently delete all wikis and users in this account.')}}
                    </p>
                    <form action="{{ route('teams.destroy', [ $team->slug ]) }}" method="POST" role="form">
                        {{ method_field('delete') }}
                        <button type="submit" class="btn btn-danger"><i class="fa fa-trash fa-fw"></i> {{_i('Yes I understand, delete my account')}}</button>
                    </form>
                </div>
			</div>
		</div>
